...
namespace MeshSimpler.Core
{
    ...
    {
        //public static int GetTriangleCount(this Node node)
        //{
        //    var triangleCount = 0;
        //    for (var i = 0; i < node.MMeshes.; i++)
        //    {
        ...
        //public static void ApplyToWorkingMesh(this Mesh mesh, ref WorkingMesh wm, Matrix4X4<float>[] bindposes = null)
        //{
    ...
    public struct WorkingMesh : IDisposable
    {
        ...
        public Vector3D<double>[] vertices
        {
            get
            {
            ...
            set
            {
                ...
                m_Vertices.CopyFrom(value);
            }
        }

        ReadWriteBuffer<Vector3D<double>> m_Vertices;
        ...
        //{
        //    get { return m_Triangles.ToArray(); }
        //    set
        //    {
        //        subMeshCount = 1;
        ...
        //    }
        //}
        ReadWriteBuffer<int> m_Triangles;
        ...
        public Vector3D<double>[] normals
        {
            get { return m_Normals.ToArray(0, normalsCount); }
            set
            {
                ...
                else
                {
                    ...
                    m_Normals.CopyFrom(value);
                }
            }
        }
        ReadWriteBuffer<Vector3D<double>> m_Normals;
        ...
        public Vector4D<double>[] tangents
        {
            get { return m_Tangents.ToArray(0, tangentsCount); }
            set
            {
                if (value == null || value.Length == 0)
                {
                ...
                else
                {
                    ...
                    m_Tangents.CopyFrom(value);
                }
            }
        }
        ReadWriteBuffer<Vector4D<double>> m_Tangents;
        ...
        public Vector2D<double>[] uv
        {
            get { return m_UV.ToArray(0, uvCount); }
            set
            {
                if (value == null || value.Length == 0)
                {
        ...
        ReadWriteBuffer<Vector2D<double>> m_UV;
        ...
        public Vector2D<double>[] uv2
        {
            get { return m_UV2.ToArray(0, uv2Count); }
            set
            {
                if (value == null || value.Length == 0)
                {
                ...
                else
                {
                    ...
                    m_UV2.CopyFrom(value);
                }
            }
        }
        ReadWriteBuffer<Vector2D<double>> m_UV2;
        ...
        public Vector2D<double>[] uv3
        {
            get { return m_UV3.ToArray(0, uv3Count); }
            set
            {
                if (value == null || value.Length == 0)
                {
                ...
                else
                {
                    ...
                    m_UV3.CopyFrom(value);
                }
            }
        }
        ReadWriteBuffer<Vector2D<double>> m_UV3;
        ...
        public Vector2D<double>[] uv4
        {
            get { return m_UV4.ToArray(0, uv4Count); }
            set
            {
                if (value == null || value.Length == 0)
                {
                ...
                else
                {
                    ...
                    m_UV4.CopyFrom(value);
                }
            }
        }
        ReadWriteBuffer<Vector2D<double>> m_UV4;
        ...
        public Vector4D<double>[] colors
        {
            get { return m_Colors.ToArray(0, colorsCount); }
            set
            {
                if (value == null || value.Length == 0)
                {
                ...
                else
                {
                    ...
                    m_Colors.CopyFrom(value);
                }
            }
        }
        ReadWriteBuffer<Vector4D<double>> m_Colors;
        ...
        public Matrix4X4<double>[] bindposes
        {
            get { return m_Bindposes.ToArray(0, bindposesCount); }
            set
            {
                if (value == null || value.Length == 0)
                {
                ...
                else
                {
                    ...
                    m_Bindposes.CopyFrom(value);
                }
            }
        }
        ReadWriteBuffer<Matrix4X4<double>> m_Bindposes;
        ...
        ReadWriteBuffer<int> m_SubmeshOffset;
        ...
        public string name
        {
            get { return Encoding.UTF8.GetString(m_Name.ToArray(0, subMeshCount)); }
            set
            {
                if (value == null)
                    value = string.Empty;
                ...
                var length = Math.Min(bytes.Length, k_MaxNameSize);
                m_Name.CopyFrom(bytes);
            }
        }
        ReadWriteBuffer<byte> m_Name;
        ...
        //        var sourceSlice = NativeSlice<int>(m_Triangles, postSliceOffset, postSliceLength);
        ...
        public WorkingMesh(int maxVertices, int maxTriangles, int maxSubmeshes, int maxBindposes) : this()
        {
            m_Counts = Gpu.Default.AllocateReadWriteBuffer<int>(Enum.GetValues(typeof(Channel)).Length, AllocationMode.Default);
            m_Vertices = Gpu.Default.AllocateReadWriteBuffer<Vector3D<double>>(maxVertices, AllocationMode.Default);
            m_Normals = Gpu.Default.AllocateReadWriteBuffer<Vector3D<double>>(maxVertices, AllocationMode.Default);
            m_Tangents = Gpu.Default.AllocateReadWriteBuffer<Vector4D<double>>(maxVertices, AllocationMode.Default);
            m_UV = Gpu.Default.AllocateReadWriteBuffer<Vector2D<double>>(maxVertices, AllocationMode.Default);
            m_UV2 = Gpu.Default.AllocateReadWriteBuffer<Vector2D<double>>(maxVertices, AllocationMode.Default);
            m_UV3 = Gpu.Default.AllocateReadWriteBuffer<Vector2D<double>>(maxVertices, AllocationMode.Default);
            m_UV4 = Gpu.Default.AllocateReadWriteBuffer<Vector2D<double>>(maxVertices, AllocationMode.Default);
            m_Colors = Gpu.Default.AllocateReadWriteBuffer<Vector4D<double>>(maxVertices, AllocationMode.Default);
            //m_BoneWeights = new NativeArray<BoneWeight>(maxVertices, AllocationMode.Default);
            m_Bindposes = Gpu.Default.AllocateReadWriteBuffer<Matrix4X4<double>>(maxBindposes, AllocationMode.Default);
            m_Name = Gpu.Default.AllocateReadWriteBuffer<byte>(k_MaxNameSize, AllocationMode.Default);
            m_SubmeshOffset = Gpu.Default.AllocateReadWriteBuffer<int>(maxSubmeshes, AllocationMode.Default);
            m_Triangles = Gpu.Default.AllocateReadWriteBuffer<int>(maxTriangles, AllocationMode.Default);
        }
